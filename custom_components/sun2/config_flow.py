...
def config_entry_params(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    """Get config entry parameters from configuration data."""
    data = data.copy()
    # translations = await async_get_translations(hass, hass.config.language, "service_name", [DOMAIN], False)
...
class Sun2ConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_import(self, data: dict[str, Any]) -> FlowResult:
        ...
        return self.async_create_entry(data={}, **config_entry_params(self.hass, data))
